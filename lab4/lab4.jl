...
function agent_count(world::World)
    ...
    for (id,agent) in world.agents
        #key = supertype(typeof(agent))
        key = typeof(agent).parameters[1]
        if haskey(count,key)
...
Sheep(id; E=4.0, ΔE=0.2, pr=0.8, pf=0.6, s=rand(Bool) ? Female : Male) = Sheep(id, E, ΔE, pr, pf, s)
Wolf(id; E=10.0, ΔE=8.0, pr=0.1, pf=0.2, s=rand(Bool) ? Female : Male) = Wolf(id, E, ΔE, pr, pf, s)
...
using Plots
plt = plot()
tolabel(::Type{Sheep}) = "Sheep"
tolabel(::Type{Wolf}) = "Wolf"
tolabel(::Type{Grass}) = "Grass"
for (A,c) in counts
